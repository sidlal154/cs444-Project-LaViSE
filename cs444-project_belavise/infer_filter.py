...
def inference(args: argparse.Namespace,
    ...
    for u, max_imgs_sorted_u in max_imgs_sorted.items():
        ...
        # [CS444: modifications to include BERT embeddings]:
        tokens = bert_tokenizer.convert_ids_to_tokens(words.cpu())
        # [CS444: mod ends]
...
def main(args: argparse.Namespace):
    ...
    tokens = tokenizer(vocab_list, return_tensors='pt', padding=True, truncation=True)
    with torch.no_grad():
        outputs = bert_model(**tokens)
    last_hidden_states = outputs.last_hidden_state
    embeddings = last_hidden_states.mean(dim=1).squeeze(0).T.cuda()
    ...
    with torch.no_grad():
        inference(args, model, dataloader, tokenizer, embeddings,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--word-embedding-dim", type=int, default=768,
