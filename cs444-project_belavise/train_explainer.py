...
def main(args: argparse.Namespace):
    ...
    if args.refer == "coco":
        ...
        embeddings = []
        for idx in glove_indices:
            print("Processing BERT embeddings: ",idx)
            embeddings.append(bert_model(torch.tensor([[idx]]))[0].mean(dim=1).squeeze().tolist())
        print(embeddings)
        embeddings = torch.tensor(embeddings).T.cuda()
        #[CS444: mod ends]
    else:
